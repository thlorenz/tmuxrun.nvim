...
local function activePaneIndex()
	return tonumber(tmux.sendTmuxCommand('display-message -p "#{pane_index}"'))
end

-- @returns list of tmux panes of the current window
local function tmuxPanes()
	local panes = tmux.sendTmuxCommand("list-panes")
	return utils.splitOnNewline(panes)
end
...
function windowMap()
	local windowMap = {}
	local result = tmux.sendTmuxCommand("list-windows")
	local lines = utils.splitOnNewline(result)
...
local function tmuxInfo(message)
	...
	return tmux.sendTmuxCommand("display-message -p '#{" .. message .. "}'")
end
...
function M._sendKeys(self, keys)
	...
	return tmux.sendTmuxCommand(fullCmd)
end
...
function M.createRunnerPane(self, config)
	...
	local result = tmux.sendTmuxCommand(cmd)
	if result ~= nil and result ~= "" then
...
M:init({ initialCommand = "ls -la", deferSendKeysForNewRunner = 1000 })
